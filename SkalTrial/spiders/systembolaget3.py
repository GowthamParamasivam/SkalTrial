...
class Systembolaget1Spider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for sub_cat in sub_cats:
            yield scrapy.Request(
                url=f'https://www.systembolaget.se/api/productsearch/search/sok-dryck/?subcategory=Aperitif%20och%20dessert&type={sub_cat}&sortfield=Name&sortdirection=Ascending&fullassortment=1&page=0&nofilters=1',
                callback=self.parse
            )
    ...
    def parse(self, response):
        ...
        cat = parse_qs(urlparse(str(response.request.url)).query)['type'][0]
        ...
        if next_page != -1:
            yield scrapy.Request(
                url=f"https://www.systembolaget.se/api/productsearch/search/sok-dryck/?subcategory=Aperitif%20och%20dessert&type={cat}&sortfield=Name&sortdirection=Ascending&fullassortment=1&page={next_page}&nofilters=1",
                callback=self.parse
            )
