...
class Systembolaget1Spider(scrapy.Spider):
    ...
    def start_requests(self):
        sub_cats = ['Montilla','Mj%C3%B6d']
        # sub_cats = ['Bitter']
        for sub_cat in sub_cats:
            yield scrapy.Request(
                url=f'https://www.systembolaget.se/api/productsearch/search/sok-dryck/?style={sub_cat}&subcategory=Aperitif%20och%20dessert&type=Smaksatta%20viner%20%26%20fruktviner&sortfield=Name&sortdirection=Ascending&fullassortment=1&page=0&nofilters=1',
                callback=self.parse
            )
    ...
    def parse(self, response):
        ...
        cat = parse_qs(urlparse(str(response.request.url)).query)['style'][0]
        ...
        for product in products:
            ...
            Item['CountryFlagUrl'] = 'https://www.systembolaget.se/content/assets/images/flags/'+str(Item['Country']).lower()+'.gif'
        ...
        if next_page != -1:
            yield scrapy.Request(
                url=f"https://www.systembolaget.se/api/productsearch/search/sok-dryck/?style={cat}&subcategory=Aperitif%20och%20dessert&type=Smaksatta%20viner%20%26%20fruktviner&sortfield=Name&sortdirection=Ascending&fullassortment=1&page={next_page}&nofilters=1",
                callback=self.parse
            )
